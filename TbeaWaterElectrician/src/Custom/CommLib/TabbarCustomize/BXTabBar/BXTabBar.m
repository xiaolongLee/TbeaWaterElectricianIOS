...
{
//	DLog("button.tag====%d",button.tag - BXTabBarTag);
	if(button.tag - BXTabBarTag == 4)
	{
		if(![AddInterface judgeislogin])
		{
			if ([_delegate respondsToSelector:@selector(personpagepoplogin)])
			{
				[_delegate personpagepoplogin];
			}
		}
		else
		{
