...
+ (void)setupNavigationBarTheme {
    ...
    // 1.设置导航条的背景
    [appearance setBackgroundImage:[UIImage createImageWithColor:ColorBlue] forBarMetrics:UIBarMetricsDefault];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    if (self.viewControllers.count > 0) {// 如果现在push的不是栈底控制器(最先push进来的那个控制器)
        ...
        //设置导航栏的按钮
//       UIBarButtonItem *backButton = [UIBarButtonItem itemWithImageName:@"regiest_back" highImageName:@"regiest_back" target:self action:@selector(back)];
