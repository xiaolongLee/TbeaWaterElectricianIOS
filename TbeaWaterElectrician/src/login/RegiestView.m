...
-(void)initview
{
	...
	[self addSubview:imageviewtopblue];
...
-(UIView *)cardpic:(UIView *)inpuview Scrollview:(UIScrollView *)scrollview
{
	...
	//身份证示例
	UIImageView *imagecard1 = [[UIImageView alloc] init];
	imagecard1.image = LOADIMAGE(@"regiest_card1", @"png");
	[cardpicview addSubview:imagecard1];
	[imagecard1 mas_makeConstraints:^(MASConstraintMaker *make) {
		make.top.mas_equalTo(imagewhitebg.mas_top).offset(nowheight);
		make.size.mas_equalTo(CGSizeMake(80, 50));
		make.left.mas_equalTo(imageaddcard1.mas_centerX).offset(-40);
	}];
	
	UIImageView *imagecard2 = [[UIImageView alloc] init];
	imagecard2.image = LOADIMAGE(@"regiest_card2", @"png");
	[cardpicview addSubview:imagecard2];
	[imagecard2 mas_makeConstraints:^(MASConstraintMaker *make) {
		make.top.mas_equalTo(imagecard1.mas_top);
		make.size.mas_equalTo(imagecard1);
...
{
	UIImageView *imageview1 = [self viewWithTag:EnRegiestNumber2];
	UILabel *label1 = [self viewWithTag:EnRegiestLable2];
	[imageview1 setImage:LOADIMAGE(@"regiest_2gray", @"png")];
	label1.textColor = COLORNOW(208, 208, 208);
	UIImageView *imageline = [self viewWithTag:EnRegiestLine1];
	...
	[[self viewWithTag:EnRegiestLeftView] removeFromSuperview];
	[[self viewWithTag:EnRegiestRightView] removeFromSuperview];
	[self telcertificationview:viewtop];
}
...
//	UIView *viewtemp = sender.view;
	...
	[[self viewWithTag:EnViewLookImage] removeFromSuperview];
}
...
-(void)selectpic:(UIGestureRecognizer*)sender
{
	...
	{
		[JPhotoMagenage getOneImageInController:self.homepage finish:^(UIImage *images) {
			NSLog(@"%@",images);
		} cancel:^{
...
{
	UITextField *textfield = (UITextField *)[self viewWithTag:EnRegiestPwdTextFieldTag];
	UITextField *textfield1 = (UITextField *)[self viewWithTag:EnRegiestPhoneTextFieldTag];
	UITextField *textfield2 = (UITextField *)[self viewWithTag:EnRegiestCodeTextFieldTag];
	UITextField *textfield3 = (UITextField *)[self viewWithTag:EnRegiestCarRealNameTextFieldTag];
	UITextField *textfield4 = (UITextField *)[self viewWithTag:EnRegiestCardNumberTextFieldTag];
	[textfield resignFirstResponder];
...
-(void)clickregiest
{
	UITextField *textfield1 = (UITextField *)[self viewWithTag:EnRegiestCarRealNameTextFieldTag];
	UITextField *textfield2 = (UITextField *)[self viewWithTag:EnRegiestCardNumberTextFieldTag];
	if(([[textfield1 text] length]==0)||([[textfield2 text] length]==0))
	{
		[MBProgressHUD showError:@"请填真实姓名和身分证书号" toView:self];
	}
	else
	{
		[self requestregiest];
...
-(void)getnextstep:(id)sender
{
	...
	{
		[MBProgressHUD showError:@"请填写电话和密码和验证码" toView:self];
	}
	else
	{
...
-(void)getphonecode:(id)sender
{
	...
	{
		UITextField *textfield1 = [self viewWithTag:EnRegiestPhoneTextFieldTag];
...
{
	UIButton *button = (UIButton *)[self viewWithTag:7690];
...
-(void)clickgetcode
{
	...
	[RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:@"TBEAENG001001001000" ReqUrl:URLHeader ShowView:self alwaysdo:^
	 {
...
-(void)requestregiest
{
	...
	[arrayimage addObject:LOADIMAGE(@"testpic", @"png")];
	[arrayimage addObject:LOADIMAGE(@"testpic2", @"png")];
	[arrayimage addObject:LOADIMAGE(@"testpic3", @"png")];
	[RequestInterface doGetJsonWithArraypic:arrayimage Parameters:params App:app RequestCode:@"TBEAENG001001002000" ReqUrl:URLHeader ShowView:self alwaysdo:^{
