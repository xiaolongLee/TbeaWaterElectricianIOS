//
//  SelectAddrNotAddressViewController.m
...
#import "SelectAddrNotAddressViewController.h"

@interface SelectAddrNotAddressViewController ()

@end

@implementation SelectAddrNotAddressViewController
...
- (void)viewDidLoad {
	...
	UIButton *btreturnright = [UIButton buttonWithType:UIButtonTypeCustom];
	btreturnright.frame = CGRectMake(SCREEN_WIDTH-50, 22, 40, 40);
	[btreturnright setTitle:@"保存" forState:UIControlStateNormal];
	btreturnright.titleLabel.font = FONTMEDIUM(14.0f);
	[btreturnright setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
	[btreturnright addTarget:self action: @selector(clicksave) forControlEvents: UIControlEventTouchUpInside];
	[self.view addSubview:btreturnright];
	...
	else
	{
		if([self.delegate1 respondsToSelector:@selector(ReceiveAddrselectAddr:)])
		{
			NSDictionary *addr = [NSDictionary dictionaryWithObjectsAndKeys:sproviceid,@"sproviceid",
									 ...
									 scityname,@"scityname",
									 sareaname,@"sareaname",nil];
			[self.delegate1 ReceiveAddrselectAddr:addr];
			[self.navigationController popViewControllerAnimated:YES];
		}
	}
}
...
-(void)initview
{
	...
	for(int i=0;i<3;i++)
	{
		...
		UITextField *textfield = [self textfieldaddr:CGRectMake(30, 64+30+40*i, SCREEN_WIDTH-60, 35) PlaceStr:placestr];
