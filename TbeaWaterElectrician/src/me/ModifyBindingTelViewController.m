//
//  ModifyBindingTelViewController.m
...
#import "ModifyBindingTelViewController.h"

@interface ModifyBindingTelViewController ()

@end

@implementation ModifyBindingTelViewController
...
-(void)yanzhengoldtel:(NSString *)tel Code:(NSString *)code
{
	...
	[RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:@"TBEAENG005001005000" ReqUrl:URLHeader ShowView:self.view alwaysdo:^
	 ...
	 Success:^(NSDictionary *dic)
	 {
		 ...
		 if([[dic objectForKey:@"success"] isEqualToString:@"true"])
		 {
			 ModifyBindingNewTelViewController *newtel = [[ModifyBindingNewTelViewController alloc] init];
			 [self.navigationController pushViewController:newtel animated:YES];
		 }
		 else
		 {
			 [MBProgressHUD showError:[dic objectForKey:@"msg"] toView:app.window];
